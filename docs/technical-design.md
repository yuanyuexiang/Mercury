...
- `GET /api/leads/export?grade=&status=` → CSV 导出（中文表头 + BOM，含渠道列，上限 5000 行）。
...
- `GET /api/metrics/overview?tz_offset_minutes=` → 窗口统计 + `funnel`（会话→线索→高意向→已同步→已成交）+ `channels`（渠道归因聚合，NULL=直接进入）+ `today`（按前端时区）+ `trend`（14 天按日会话/线索）+ `pending_handoffs`。
...
- **双槽位模型配置**（2026-08-31 修订，migration 0009）：服务商行只承载凭据（name/base_url/key），"谁来对话"与"谁来检索"是两个显式槽位——`is_active` = 对话槽、`is_embed_active` = 检索槽（各有 partial unique index，租户内至多一个），可指向不同服务商（如智谱管对话 + 硅基流动管检索）。指派走 `PUT /api/settings/llm-providers/roles/{chat|embed}`（body：provider_id + model，写入行的 chat_model/embed_model 并翻转槽位标志，广播失效热生效）；`ProviderSource.get_embed()` 解析：检索槽行 → env 兜底。担任槽位的服务商也可删除——删除即腾空槽位并广播失效（前端弹窗说明后果；对话槽空缺时机器人走"系统未就绪"降级）。后台页面为微信式工作台（与会话页同构）：左栏服务商列表（置顶「当前生效」总览项，`?id=` 深链）+ 右侧内容区（总览=两张槽位状态卡+用量图；选中服务商=密钥管理+「设为对话/设为检索」两个配置块）。自动化三层：进入服务商详情自动拉取模型列表并按命名规则分类（`web/src/lib/providers.ts::classifyModels`，embedding 关键词→检索、语音/图像/rerank 排除、其余对话——分流到对应下拉，选错槽位成为不可能，拿不准的进「其他」分组不隐藏）；空输入自动预填（预设推荐优先，其次分类首项）；保存槽位自动实测生效。
...
**渠道归因**（migration 0006）：客户投放时使用深链 `t.me/<bot>?start=<渠道标识>`（字符集 `[A-Za-z0-9_-]{1,64}`，其余丢弃）；`/start` 首触写入 `conversations.source_channel`（已有不覆盖），线索创建时继承到 `leads.source_channel`，Sheets 同步含 Channel 列，概览页按渠道聚合会话/线索/高意向。
