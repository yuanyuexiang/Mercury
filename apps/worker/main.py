...
from llm.brain import build_brain
...
async def on_startup(ctx: dict[str, Any]) -> None:
    ...
    ctx["embedder"] = build_embedder(settings)  # 无 key 时为 None：索引任务明确失败
    ctx["brain"] = build_brain(settings)  # 无 key/模型名时为 None：回复走安全降级文案
...
class WorkerSettings:
    functions = [process_update, index_document]
